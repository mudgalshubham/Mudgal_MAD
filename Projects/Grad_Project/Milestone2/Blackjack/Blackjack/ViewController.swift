...
class ViewController: UIViewController {
    ...
    func updateScore(){
        ...
        databaseref = databaseref.child("scores").child("win")
        
        databaseref.observeSingleEventOfType(.Value, withBlock:{
            snapshot in
            
            var currentWin: String
            currentWin = String(snapshot.value!) as String
            
            
            print(currentWin)
            
            
            let a:Int? = Int(currentWin)
    ...
    func updateDatabase() -> (){
        ...
        var q: FIRDatabaseReference
        
        q = databaseref.child("scores")
        
        q.observeSingleEventOfType(.Value, withBlock:{
            snapshot in
            
            var currentScore: String
            currentScore = String(snapshot.value!) as String
            
            print(currentScore)
            let a:Int? = Int(currentScore)
            
            var newScore :Int = a!
            
            newScore = newScore + 1
            q.setValue(String(newScore))
            }, withCancelBlock: nil)
