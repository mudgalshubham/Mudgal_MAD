...
class ViewController: UIViewController {
    ...
    @IBAction func resetButton(sender: UIBarButtonItem) {
        ...
        let alert = UIAlertController(title: "Alert", message:"Are you sure you want to reset the score?", preferredStyle: UIAlertControllerStyle.Alert)
        ...
        let okAction = UIAlertAction(title: "Yes", style: UIAlertActionStyle.Default, handler:
            {
                ...
                self.updateWinsInDB(0)
                self.updateTotalInDB(0)
    ...
    func updateScoreBoard() {
        //update scoreboard to show number of games the player has won
        self.ScoreBoard.text = "\(self.wins)" + "/" + "\(self.gamesPlayed)"
    }
    ...
    func gameOverMessage(messageTitle: String, messageContent: String){
        self.gamesPlayed += 1
        updateTotalInDB(self.gamesPlayed)
    ...
    override func viewDidLoad() {
        ...
        // update scores from firebase Database
        updateDatabase()
    ...
    // update database with total score
    func updateTotalInDB(newVal : Int){
        var databaseref = FIRDatabase.database().reference()
        databaseref = databaseref.child("scores").child("total games")
        databaseref.setValue(newVal)
    }
    
    // update database with number of wins
    func updateWinsInDB(newVal : Int){
        var databaseref = FIRDatabase.database().reference()
        databaseref = databaseref.child("scores").child("wins")
        databaseref.setValue(newVal)
    }
    
    // function to update game's score with database score
    func updateDatabase(){
        
        var databaseref = FIRDatabase.database().reference()
        
        databaseref = databaseref.child("scores")
        let dbWins = databaseref.child("wins")
        let dbTotal = databaseref.child("total games")
        
        dbWins.observeSingleEventOfType(.Value, withBlock:{
            snapshot in
            
            var currentWins: String
            
            currentWins = String(snapshot.value!) as String
            
            let newWin:Int? = Int(currentWins)
        ...
        dbTotal.observeSingleEventOfType(.Value, withBlock:{
            snapshot in
            
            var currentTotal: String
            
            currentTotal = String(snapshot.value!) as String
            
            let newTotal:Int? = Int(currentTotal)
            
            
            if(newTotal != nil){
                self.gamesPlayed = newTotal!
            }
            self.updateScoreBoard()
            }, withCancelBlock: nil)
